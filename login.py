...
class Login(ABC):
    def __init__(
        ...
        **kwargs,
        ...
        self.scan_qrcode_timeout = kwargs.get("scan_qrcode_timeout") or 60
    ...
    async def begin(self):
        ...
        # check login state
        self.logger.info(f"finish login then checking login state ...")
        try:
            await self.check_login_state()
        except RetryError:
            self.logger.fatal("failed to login ...")
    ...
    async def popup_login_dialog(self):
        ...
        try:
            ...
            self.logger.warn(
                f"login dialog box does not pop up automatically, error: {e}, click manually"
            )
    ...
    async def login_by_qrcode(self):
        self.logger.info("try login by qrcode...")
        qrcode_img_selector = "xpath=//article[@class='web-login']//img"
        base64_qrcode_img = await find_login_qrcode(
            self.logger, self.context_page, selector=qrcode_img_selector
        )
        if not base64_qrcode_img:
            self.logger.error("login qrcode not found")
            sys.exit()
        ...
        show_qrcode(base64_qrcode_img)
        await asyncio.sleep(2)

    async def login_by_mobile(self):
        self.logger.info("try login by mobile ...")
        ...
        self.logger.info("waiting to enter the verification code")
        max_get_sms_code_time = 60 * 2  # 最长获取验证码的时间为2分钟
        retry_count = 0
        while max_get_sms_code_time > 0:
            if retry_count > 0:
                self.logger.info(
                    f"verification time remaining {max_get_sms_code_time}s ..."
                )
                await asyncio.sleep(1)
            data = input("please enter the verification code: ")
            if "exit" == data.lower():
                break
            retry_count += 1
            ...
            await sms_code_input_ele.fill(value=data.rstrip())
            await asyncio.sleep(0.5)
    ...
    async def check_page_display_slider(
        ...
        while not slider_verify_success:
            if max_slider_try_times <= 0:
                self.logger.fatal("slider verify failed ...")
            try:
                ...
                if "操作过慢" in page_content or "提示重新操作" in page_content:
                    self.logger.info("retrying verify slider ...")
                ...
                self.logger.info("slider verify successfully ...")
                slider_verify_success = True
            except Exception as e:
                self.logger.error(f"failed to verify slider, error: {e}")
                await asyncio.sleep(1)
                max_slider_try_times -= 1
                self.logger.debug(
                    f"retrying verify slider remaining times: {max_slider_try_times}"
                )
                continue
    ...
    async def login_by_cookies(self):
        self.logger.info("trying login by cookie ...")
